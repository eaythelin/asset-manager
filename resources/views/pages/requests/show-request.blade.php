  ...
  <div class="bg-white p-4 rounded-2xl shadow-2xl mt-4">
    ...
    <div class = "flex flex-col sm:flex-row gap-6">
      <div class = "flex flex-col flex-1 gap-4">
        <div class="form-row">
          ...
          <x-page-input value="{{ $requestModel->requestedBy->name }}" id="requisitioner" name="requisitioner" readonly/>
        </div>

        <div class="form-row">
          <x-page-label for="asset" :required="true">Equipment/Vehicle</x-page-label>
          <x-page-input value="{{ $requestModel->asset->name }}" id="asset" name="asset" readonly/>
        </div>

        <div class="form-row">
          <x-page-label for="description" :required="true">Request Description/Plate No.</x-page-label>
          <x-page-textarea name="description" id="description">{{ $requestModel->description }}</x-page-textarea>
        </div>
      </div>
    ...
    <div class="flex flex-col sm:flex-row gap-6 mt-10">
      ...
      <div class="form-row flex-1">
        ...
        <x-page-input value="{{ $requestModel->approvedBy?->name }}" name="approved_by" id="approved_by" disabled/>
      </div>
    </div>
  </div>
</div>
